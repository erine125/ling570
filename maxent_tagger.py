...
class POS_Tagger(object):
    ...
    def create_train_voc(self):
        token_list_by_line = [] #list of lists of tokens in each line
        self.word_freq_dict = {}
        ...
        with open(self.train_file, 'r') as file:
            for line in file:
                ...
                for element in words_tags:
                    ...
                    curr_line.append(word_tag_tuple)
                token_list_by_line.append(curr_line)

        #### Create a dictionary with word:freq

        idx = 0 #idx represents the unique index of each token
        for line in token_list_by_line:
            for j in range(len(line)):
                ...
                if j >= 1:
                    tag_minus_1 = line[j-1][1]
                    word_minus_1 = line[j-1][0]
                else:
                    tag_minus_1 = "BOS" 
                    word_minus_1 = "</s>"

                if j >= 2:
                    tag_minus_2 = line[j-2][1]
                    word_minus_2 = line[j-2][0]
                else:
                    tag_minus_2 = None 
                    word_minus_2 = None

                if j+1 < len(line):
                    word_plus_1 = line[j+1][0]
                else:
                    word_plus_1 = "<s>" 

                if j+2 < len(line):
                    word_plus_2 = line[j+2][0]
                else:
                    word_plus_2 = None
                ...
                self.indexed_tag_dict[idx] = tags_list
                self.indexed_word_dict[idx] = words_list
    ...
    def create_test_vectors(self):
        ...
        for line in token_list_by_line:
            for j in range(len(line)):
                ...
                self.test_indexed_tag_dict[idx] = tags_list
                self.test_indexed_word_dict[idx] = words_list
        ...
        for i in range(len(self.test_indexed_word_dict)):
            ...
            next2W = self.test_indexed_word_dict[i][4]
            if next2W != None:
    ...
    def create_init_feats(self):
        ...
        for i in self.indexed_word_dict:
            ...
            next2W = self.indexed_word_dict[i][4]
            if next2W != None:
                next2W_key = "next2W="+next2W
    ...
    def isRare(self, word):
        ...
        try:
            freq = self.word_freq_dict[word]
        except KeyError:
            freq = 0 # if word not in dict, then its frequency is 0
            

        if freq < self.rare_thres:
            return True
        else:
            return False 
...
def main():
    ...
    print(len(tagger.init_feat_freqs))
    print(len(tagger.kept_feat_freqs))
